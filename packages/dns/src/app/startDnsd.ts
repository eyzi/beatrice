...
) => !repository ? [] : queryRecord(repository, { type: "SOA" })
	.then(records => records.map(record => {
		const { mname, rname, serial, refresh, retry, expire, ttl } = record.data as ZoneData;
		dnsServer.zone(record.name, mname, rname, serial, refresh, retry, expire, ttl);
	}))
...
const handleQuestion = async (
	...
	return createAnswer(repository, question.name as string, question.type)
		.then(mapAnswers(question.name as string));
}
...
const requestHandler = (
	...
	for (const question of questionArray) {
		const rawAnswers = await handleQuestion(question, repository);
