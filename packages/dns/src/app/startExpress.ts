...
const handleQueryRecord =
  ...
  async (req: Request, res: Response) => {
    ...
    if (Object.keys(req.query).length > 0) {
      const records = await queryRecord(repository, req.query as RecordQuery);
      res.json(records);
    } else {
      res.end("pong");
    }
...
const handleDeleteQueryRecord =
  (repository?: Repository<Record, RecordQuery>) =>
  async (req: Request, res: Response) => {
    if (!repository) return serverError(res);
    if (Object.keys(req.query).length > 0) {
      const foundRecords = await queryRecord(
        repository,
        req.query as RecordQuery
      );
      if (!foundRecords || foundRecords.length < 1)
        return res.status(404).end("DNS record not found");

      const foundRecord = foundRecords[0];
      const record = await deleteRecord(repository, foundRecord.id);
