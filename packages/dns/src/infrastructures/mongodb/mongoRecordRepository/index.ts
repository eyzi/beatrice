...
import { Id, OmitId, Repository } from "@beatrice/common";
...
const initCreate =
  (db: Connection) =>
  async (body: OmitId<Record>): Promise<Record> => {
...
const initGet =
  (db: Connection) =>
  async (id: Id): Promise<Record | null> => {
    if (!Types.ObjectId.isValid(id)) return null;
    const RecordModel = db.model<RecordDocument>(COLLECTION_NAME, RecordSchema);
    return RecordModel.findById(id).then(buildRecordOrNull);
  };

const initGetAll = (db: Connection) => async (): Promise<Record[]> => {
  const RecordModel = db.model<RecordDocument>(COLLECTION_NAME, RecordSchema);
  return RecordModel.find({}).then((docs: RecordDocument[]) =>
    docs.map(buildRecord)
  );
};

const initQuery =
  (db: Connection) =>
  async (recordQuery: RecordQuery): Promise<Record[]> => {
    const query = {
      type: recordQuery.type,
      ...(recordQuery.data ? { data: recordQuery.data } : undefined),
      ...(recordQuery.name
        ? {
            $or:
              typeof recordQuery.name === "string"
    ...
    const RecordModel = db.model<RecordDocument>(COLLECTION_NAME, RecordSchema);
    return RecordModel.find(query).then((docs: RecordDocument[]) =>
      docs.map(buildRecord)
    );
  };
...
const initDelete =
  (db: Connection) =>
  async (entity: Record | Id): Promise<boolean> => {
    const id = typeof entity === "string" ? entity : entity.id;
    if (!Types.ObjectId.isValid(id)) return false;

    const RecordModel = db.model<RecordDocument>(COLLECTION_NAME, RecordSchema);
    return RecordModel.findByIdAndDelete(id).then((doc) => !!doc);
  };

export default (db: Connection): Repository<Record, RecordQuery> => ({
