...
const handleRenewCertificate = (
	renewer: (account: AcmeAccount, force: boolean) => Promise<CertificateRenewerReturn>,
	...
	const { renewed, message } = await renewer(acmeAccount, req.query.force === "true")
	return renewed === true ? res.status(201).end() : res.status(422).end(message)
}
