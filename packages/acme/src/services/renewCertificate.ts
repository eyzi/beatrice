import { persistenceGet, persistenceUpdate, SSL } from "@beatrice/common";
import {
  AcmeAccount,
  CertificateRenewer,
...
export default async ({
  ...
  const ssl: SSL | null = await persistenceGet(certificateRepository)(
    acmeAccount.id
  );
  const certificate = await parseCertificate(certificateParser)(
    ssl?.certificate
  );
  ...
  if (!newSSL)
    return {
      ...acmeAccount,
      renewed: false,
      message: "Certificate generator failed",
    };

  console.log(
    `Renewed certificate for domains ${acmeAccount.domains.toString()}`
  );
